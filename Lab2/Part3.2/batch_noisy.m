...
f_sin = sin(2*x) + sqrt(0.1)*randn(length(x),1); % sin(2x) (target 1)
f_sq = 1*(f_sin>=0) -1 * (f_sin<0) + sqrt(0.1)*randn(length(x),1); % square(2x) (target 2)

f_sin_val = sin(2*x_val) + sqrt(0.1)*randn(length(x_val),1); % sin(2x) (validation)
f_sq_val = 1*(f_sin_val>=0) -1 * (f_sin_val<0) + sqrt(0.1)*randn(length(x_val),1); % square(2x) (validation)
